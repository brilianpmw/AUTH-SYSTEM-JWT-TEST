...
import (
	...
	"github.com/tokopedia/shop-quest/internal/config"
	timeUtils "github.com/tokopedia/shop-quest/internal/pkg/helper/time"
	"github.com/tokopedia/shop-quest/internal/pkg/monitoring"
	"github.com/tokopedia/shop-quest/presentation"
	"github.com/tokopedia/tdk/go/metrics"
		...
		t.Run(tt.name, func(t *testing.T) {
			config.InitTestingConfig(&config.Configuration{
				...
				NewRelic: config.NewRelicConfig{
					Connection: "localhost:4565",
				},
			})
			monitoring.InitWithArgs(&metrics.Client{})
			router := chi.NewRouter()
			hd := New(router, tt.fields.usecase, tt.timeUtils)
			urlPath := "/internal/shopquest/sellergroup/assignment-history/backfill"
			hd.router.Post(urlPath, hd.HandleBackfillAssignmentHistory)
